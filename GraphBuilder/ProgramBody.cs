...
namespace GraphBuilder
{
    public partial class ProgramBody : Form
    {
                        ...
                        foreach (Vertexes Vertex in List_Vertexes) if ((Vertex.X <= e.X & e.X <= Vertex.X + Vertex.Width) & (Vertex.Y <= e.Y & e.Y <= Vertex.Y + Vertex.Height))
                            {
                                if (TemporaryUpdate == null) { TemporaryUpdate = Vertex; Draw.Vertex(Main, TemporaryUpdate, Color.Gold); }
                                        ...
                                        {
                                            EdgesResize.Sized = Edge.Size; EdgesResize.Shift = Edge.Distortion; new EdgesResize().ShowDialog(); Edge.ReSize(EdgesResize.Sized); Edge.SetDistortion(EdgesResize.Shift);
                                            Draw.FullDrawing(Main, List_Vertexes, List_Edges); TemporaryUpdate = null; DrawTemporary();
                                        }
                            }
                    }
                    break;
        ...
        { if (MessageBox.Show("Уверены, что хотите удалить выбранную вершину?", "Подтверждение", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question) == DialogResult.Yes) ProgramBody_Load(sender, e); }
        ...
        private void Load_Button_Click(object sender, EventArgs e) // Не реализовано
        {
            ...
            List_Edges.Add(new Edges(List_Vertexes[3], List_Vertexes[1])); List_Edges[7].ReSize(3); List_Edges[7].SetDistortion(new Point(0, 20));
        ...
        { Matrix_Adjacencies.BackColor = SystemColors.Control; Matrix_Weights.BackColor = SystemColors.Control; Path.BackColor = SystemColors.Control; Cycle.BackColor = SystemColors.Control; }

        private void Matrix_Adjacencies_Click(object sender, EventArgs e)
        {
            Reset_Click(); Matrix_Adjacencies.BackColor = SystemColors.ScrollBar;

            OutputWindow.Text = ""; int[,] Matrix_Adjacencies_int = new int[List_Vertexes.Count, List_Vertexes.Count]; string Out = "| x |";

            for (int I1 = 0; I1 < Matrix_Adjacencies_int.GetLength(0); I1++) for (int I2 = 0; I2 < Matrix_Adjacencies_int.GetLength(1); I2++) Matrix_Adjacencies_int[I1, I2] = 0;

            for (int I1 = 0; I1 < List_Edges.Count; I1++)
            { Matrix_Adjacencies_int[Convert.ToInt32(List_Edges[I1].A.Name) - 1, Convert.ToInt32(List_Edges[I1].B.Name) - 1] = 1; Matrix_Adjacencies_int[Convert.ToInt32(List_Edges[I1].B.Name) - 1, Convert.ToInt32(List_Edges[I1].A.Name) - 1] = 1; }

            for (int I1 = -1; I1 < Matrix_Adjacencies_int.GetLength(0); I1++)
            {
                if (I1 == -1) { for (int I2 = 0; I2 < List_Vertexes.Count; I2++) Out += $" | {I2 + 1} |"; OutputWindow.Text += $"\n{Out}"; }
                else { Out = $"| {I1 + 1} |"; for (int I2 = 0; I2 < Matrix_Adjacencies_int.GetLength(1); I2++) Out += $" | {Matrix_Adjacencies_int[I1, I2]} |"; OutputWindow.Text += $"\n{Out}"; }
            }
        }

        private void Matrix_Weights_Click(object sender, EventArgs e)
        {
            Reset_Click(); Matrix_Weights.BackColor = SystemColors.ScrollBar; OutputWindow.Text = ""; int[,] Matrix_Weights_int = new int[List_Vertexes.Count, List_Vertexes.Count]; string Out = "|    -    |";

            for (int I1 = 0; I1 < Matrix_Weights_int.GetLength(0); I1++) for (int I2 = 0; I2 < Matrix_Weights_int.GetLength(1); I2++) Matrix_Weights_int[I1, I2] = 0;

            for (int I1 = 0; I1 < List_Edges.Count; I1++)
            {
                Matrix_Weights_int[Convert.ToInt32(List_Edges[I1].A.Name) - 1, Convert.ToInt32(List_Edges[I1].B.Name) - 1] = List_Edges[I1].Size;
                Matrix_Weights_int[Convert.ToInt32(List_Edges[I1].B.Name) - 1, Convert.ToInt32(List_Edges[I1].A.Name) - 1] = List_Edges[I1].Size;
            }

            for (int I1 = -1; I1 < Matrix_Weights_int.GetLength(0); I1++)
            {
                if (I1 == -1) { for (int I2 = 0; I2 < Matrix_Weights_int.GetLength(1); I2++) Out += $" x{(I2 + 1):000}x |"; OutputWindow.Text += $"\n{Out}"; }
                else { Out = $"|  {(I1 + 1):000}  |"; for (int I2 = 0; I2 < Matrix_Weights_int.GetLength(1); I2++) Out += $" {Matrix_Weights_int[I1, I2]:00000} |"; OutputWindow.Text += $"\n{Out}"; }
            }
        }

        private void Path_Click(object sender, EventArgs e)
        {
            OutputWindow.Text = ""; int[] Path = new int[List_Vertexes.Count];
        ...
        {
            if (I1 != I2) Path[I1] = 2; else { OutputWindow.Text += $"\n{Out}"; return; }
        ...
        {
            LogOut.Clear(); OutputWindow.Text = ""; int[] Path = new int[List_Vertexes.Count];
        ...
        private void Cycle_P(int I1, int I2, int[] Path, int UnavailableEdge, List<int> Cycle)
        {
            ...
            else if (Cycle.Count >= 2)
            {
                ...
                for (int I3 = 1; I3 < LogOut.Count; I3++) if (LogOut[I3].ToString() == Out) { Flag = true; break; }
                if (!Flag) { Cycle.Reverse(); Out = Cycle[0].ToString(); for (int I3 = 1; I3 < Cycle.Count; I3++) Out += $"-{Cycle[I3]}"; LogOut.Add(Out); OutputWindow.Text += $"\n{Out}"; }
                return;
            }
